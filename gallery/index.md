---
layout: page
title: Gallery
---
<style>
.pics {display:none;}
#lightbox {
    position:fixed; // keeps the lightbox window in the current viewport
    top:0;
    left:0;
    width:100%;
    height:100%;
    background: rgba(0,0,0,.5);
    text-align:center;
};
#lightbox img {
    box-shadow:0 0 25px #111;
    -webkit-box-shadow:0 0 25px #111;
    -moz-box-shadow:0 0 25px #111;
};
p {

};

</style>
<!-- scripts -->
<script>
function toggleDiv(divId) {
    $("#"+divId).toggle();
}
</script>
<script>
jQuery(document).ready(function($) {

	$('.view').click(function(e) {

		e.preventDefault();
    var caption = $(this).attr("alt");
		var image_href = $(this).attr("src");

		if ($('#lightbox').length > 0) {
      $('#content').html('<img src="' + image_href + '" style="margin-top:120px;"/><p style="color: white;font-size:16px;padding-top:10px;">' + caption + '</p>');
			$('#lightbox').show();
		}

		else {
			var lightbox =
			'<div id="lightbox">' +
				'<div id="content">' +
					'<img src="' + image_href +'" style="margin-top:120px;"/>' +
          '<p style="color: white;font-size:16px;padding-top:10px;">' + caption + '</p>' +
				'</div>' +
			'</div>';

			$('.navbar').append(lightbox);
		}
	});

	$('body').on('click', '#lightbox', function(){
		$('#lightbox').hide();
	});
});
</script>

<body>
<div class="container">
    <h2><a href="javascript:toggleDiv('jul16')">July</a></h2>
    <div class="pics" id="jul16">
      <img class="view" alt="cute-dog!" src="data:image/jpeg;base64,">
      <img class="view" alt="<3" src="https://tse1.mm.bing.net/th?id=OIP.M9bdb5b45739754e85e0ead381b5bf106H0&w=131&h=100&c=7&rs=1&qlt=90&pid=3.1&rm=2">
      <img class="view" alt="another cutie" src="https://tse1.mm.bing.net/th?id=OIP.Me88f59a1935827adf345346ecd5a1c03H0&w=130&h=108&c=7&rs=1&qlt=90&pid=3.1&rm=2">
    </div>

    <h2><a href="javascript:toggleDiv('jun16')">June</a></h2>
    <div class="pics" id="jun16">hello</div>

    <h2><a href="javascript:toggleDiv('may16')">May</a></h2>
    <div class="pics" id="may16">hello</div>
</div>
</body>
